  ...
  ResourceHistory.find({resourceType:"SecurityEvent"}, function (rhErr, histories) {
    ...
    async.forEach(histories, function(history, callback) {
      ...
      history.findLatest( function(err, securityevent) {
        var entrywrapper = {
          title: "SecurityEvent " + history.vistaId + " Version " + history.versionCount(),
          id: "http://localhost:3000/securityevent/@" + history.vistaId,
          link: {
            href: "http://localhost:3000/securityevent/@" + history.vistaId + "/history/@" + history.versionCount(),
            rel: "self"
          },
