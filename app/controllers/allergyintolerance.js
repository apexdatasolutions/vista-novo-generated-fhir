...
exports.list = function(req, res) {
  ...
  ResourceHistory.find({resourceType:"Patient"}, function (rhErr, histories) {
    ...
    if (histories !== null) {
      async.forEach(histories, function(history, callback) {
        history.findLatest( function(err, allergyintolerance) {
          var entrywrapper = {
            ...
            id: "http://localhost:3000/allergyintolerance/@" + history.latestVersionId(),
            link: {
              href: "http://localhost:3000/allergyintolerance/@" + history.latestVersionId() + "/history/@" + history.versionCount(),
              rel: "self"
            },
      ...
      console.log('no allergyintolerance found');
      res.send(500);
    }
  });
};