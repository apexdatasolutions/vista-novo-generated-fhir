  ...
  if (model === 'medication' && typeof req.params.id !== "undefined") {
    ...
    ResourceHistory.findInCacheOrLocal(medicationId, 'Medication', function(resourceHistory) {
      if (resourceHistory) {
        ...
        restler.get(requestUrl, {username: req.serviceConfig.medications.username, 
                                    password: req.serviceConfig.medications.password,
                                    authorization: req.header['authorization']}).once('complete', function(vistaMedication) {
