...
exports.checkObservationCache = function(req, res, next, model) {
  if (model === 'observation' && req.params.id !== null) {
    ...
    ResourceHistory.findInCacheOrLocal(observationId, 'Observation', function(resourceHistory) {
      if (resourceHistory) {
        ...
        restler.get(requestUrl, {username: req.serviceConfig.observations.username, password: req.serviceConfig.observations.password}).on('complete', function(vistaObservation) {
