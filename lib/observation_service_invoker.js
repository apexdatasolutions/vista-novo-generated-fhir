  ...
  if (model === 'observation' && typeof req.params.id !== "undefined") {
    ...
    ResourceHistory.findInCacheOrLocal(observationId, 'Observation', function(resourceHistory) {
      if (resourceHistory) {
        ...
        restler.get(requestUrl, {username: req.serviceConfig.observations.username, 
                                  password: req.serviceConfig.observations.password,
                                  authorization: req.header['authorization']}).once('complete', function(vistaObservation) {
